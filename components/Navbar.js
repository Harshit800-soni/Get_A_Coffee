  ...
  return (
    <nav className='bg-gray-900 shadow-xl shadow-white text-white flex justify-between items-center px-4 md:h-16'>
      ...
      <Link className="logo font-bold text-lg flex justify-center items-center" href={"/"}>
        ...
        <span className='text-xl md:text-base my-3 md:my-0'>Get A Coffee</span>
      </Link>
        ...
        {status === "authenticated" && <>
          ...
          <div id="dropdown" className={`z-10 ${showdropdown ? "" : "hidden"} absolute left-3.75 top-12 bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700`}>
            <ul className="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdownDefaultButton">
              ...
              <li>
                <button type="button" onClick={() => signOut({ callbackUrl: "/" })} className="block w-full px-4 py-2 text-left hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Sign out</button>
              </li>
            </ul>
          </div></>
        }

        {status === "authenticated" && <button className='text-white w-fit bg-linear-to-br from-purple-600 to-blue-500 hover:bg-linear-to-bl focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 ' onClick={() => { signOut({ callbackUrl: "/" }) }}>Logout</button>}
